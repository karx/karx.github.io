...
## Executive Summary
This documentation is to serve as a 'Statement of Work' or SOW for design and development of AdEngine POC to enable and enhance targeted advertising on TVs. 

The advancement in the field of IoT  Networks, Computing and Data Science has made possible Integrated systems that can really help business.
Such a system would include a means to capture, catalog and analyze data, along with tools to enforce 'business logic'.

This SOW summarizes a AdEngine system, with our own `on-premise` Hardware devices runing AdEngine edge device firmware, which would include (atleast) the following:
...
* Designing and Developement of the AdEngine Devices Infrastructure
* Designing and Developement the AdEngine Content Distribution Architecture
* Designing and Developement of on-premise devices
...
## Project Defination
The AdEngine project is broken down into following 5 modules, which would be developed in the course of this work.
...
The module takes input from the DTH set-top box, AdEngine Cloud services and Local memeory cache to render output to the TV.

The following two approches have been discussed as of now for taking `INPUT` from a DTH set-top box:
* Leveraging USB-inteface on DTH Setup boxes
* Pipeing HDMI-out from DTH Setup box


Choice of Hardware is to be detemined based on compute and network consumption. Current development to start from Raspberry Pi Zero interfaced with [HDMI to CSI-2 Bridge](https://www.mouser.in/Embedded-Solutions/Interface-Modules/_/N-5g1o?P=1y9f2kt)

### 1.2 Device and Identity Management
This module is responsible for provide a super-set of communication and access provisioning systems that provide easy management of user devices and their accounts, and others those who need that information.
This would include:
...
This module is responsible for collecting and storing Ad data, and distributing to all `on-premise` devices, based on user, geographic postion, AdEngine preferences.

* This would include a web-based app for uploading and managing content.
* Current approch uses S3 bucket to house large media files.
* Media transcoding (Out-of-scope) 

### 1.4 Realtime Stream Processing
The module responsible for taking Real-time decisions of which add to play, based on factors like, user persona, location, time, demand, geographic and demographic data.
...
This module, actually is a collection of marker based interupt mechanism that would facilitate handshake for where and when to place content.
...
We will be recording and cataloging the real time telemetry of the AdEngine, AdEngine on-premise devices and Users.
...
* Devices. 
To be installed in 4 AdEngine facility, and 50% spare**.
